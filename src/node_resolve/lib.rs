...
impl Resolver {
    ...
    fn resolve_as_file(&self, path: &Path) -> Result<PathBuf, InternalError> {
        ...
        if path.is_file() {
            self.cache
                .insert(path.to_path_buf(), Some(path.to_path_buf()));
            ...
            .map(String::from)
        {
            for ext in &self.extensions {
                ...
                if ext_path.is_file() {
                    self.cache
                        .insert(path.to_path_buf(), Some(ext_path.clone()));
                    return Ok(ext_path);
                }
            }
        }
    ...
    fn resolve_as_directory(&self, path: &Path) -> Result<PathBuf, InternalError> {
        ...
        if pkg_path.is_file() {
            ...
            match main {
                Ok(main_path) => {
                    self.cache
                        .insert(path.to_path_buf(), Some(main_path.clone()));
                    return Ok(main_path);
                }
                Err(_) => (),
            }
        ...
        match index {
            Ok(index_path) => {
                self.cache
                    .insert(path.to_path_buf(), Some(index_path.clone()));
    ...
    fn resolve_package_main(&self, pkg_path: &Path) -> Result<PathBuf, InternalError> {
        if let Some(cached) = self.cache.get(pkg_path) {
            let cached_path = cached.clone();
            match cached_path {
                Some(cached_path) => return Ok(cached_path),
                None => (),
        ...
        match main_field {
            Some(target) => {
                ...
                match result {
                    Ok(path) => {
                        self.cache
                            .insert(pkg_path.to_path_buf(), Some(path.clone()));
                        Ok(path)
                    }
                    Err(err) => Err(err),
                }
